/* =====================
    ...
    /* =====================
       Compression helpers (URI-safe LZ)
       ===================== */
    ...
    /* =====================
       Share (URL hash) with compressed payload (legacy-safe)
       ===================== */
    ...
    function tryLoadStateFromHash(){ const m=location.hash.match(/state=([^&]+)/); if(!m) return false; const token=m[1]; let st = decompressState(token); if (!st){ try { st = JSON.parse(b64decode(token)); } catch(e){ return false; } } if (typeof st.t === 'string') editor.value = st.t; modeToggle.checked = (st.m===1); mode = modeToggle.checked ? 'classic' : 'mixed'; document.body.classList.toggle('dark', st.d===1); darkToggle.checked = (st.d===1); if (typeof st.s === 'number' && !Number.isNaN(st.s)){ leftPane.style.flex=`0 0 ${st.s}px`; preview.style.flex=`1 1 auto`; try { localStorage.setItem(LS_SPLITPX, String(st.s)); } catch(e){} } if (st.x && typeof st.x === 'object') { MACROS = normalizeMacros(st.x); try { localStorage.setItem(LS_MACROS, JSON.stringify(MACROS)); } catch(e){} } modeLabel.textContent = (mode === 'classic') ? 'Classic' : 'Mixed'; modeDesc.textContent  = (mode === 'classic') ? 'Classic: each new line is rendered as display math.' : 'Mixed: type text with $inline$ and $$display$$ math.'; editor.placeholder = (mode === 'classic') ? 'Type lines of pure TeX; each line renders as display math' : 'Type text with $inline$ and $$display$$ math'; return true; }
    ...
    editor.addEventListener('keydown', (e) => {
      ...
      // Backspace between paired delimiters removes both
